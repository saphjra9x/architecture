...
$this->registerMetaTag([
    'property' => 'og:image',
    'content' => Url::to([$post['avatar']], true)
]);
...
<div class="body-left">
    ...
    <div id="LeftMainContent">
        ...
        <div class="othernews">
            ...
            <?php foreach (FunctionHelper::get_post_by_views(5) as $key => $value): ?>
                <h3 class="normal">
            ...
            <?php foreach (FunctionHelper::get_posts_by_id_desc(5) as $key => $value): ?>
                <h3 class="normal">
                    <a class="font-link-box-item iconlist"
                       href="<?= Url::to(['site/view', 'category_slug' => $category['slug'], 'content_slug' => $value['slug']]) ?>"
                       title="<?= $value['title'] ?>">
...
<script>
    $(document).ready(function () {
        ...
        $.ajax({
            url: base + 'admin/ajax/count-view-by-post?id='+<?= $post['id']?>+'&view='+view,
            type: 'post',
            data: {},
...
<div id="content-slug" style="display: none"><?= $post['slug'] ?></div>
<script>
